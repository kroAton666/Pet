(function () {
    ...
    function component(object) {
        ...
        this.search = function () {
            var _this = this;
            var query = object.search || cleanTitle(movie.title || movie.name);

            _this.reset();
            scroll.body().append(Lampa.Template.get('lampac_content_loading') || '<div class="broadcast__scan"><div></div></div>');
        ...
        this.draw = function (results) {
            ...
            if (results.length === 0) {
                _this.empty("Нічого не знайдено на AniTube.");
                return;
            }

            var grid = $('<div class="category-full" style="padding: 20px 0;"><div class="cards-grid" style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: flex-start; padding: 20px;"></div></div>');
            var container = grid.find('.cards-grid');

            results.forEach(function (itemData) {
                var card = $('<div class="card selector" style="width: 150px; margin: 10px; cursor: pointer; position: relative;">' +
                    '<div class="card__view" style="position: relative; width: 150px; height: 220px; border-radius: 8px; overflow: hidden; background: #1a1a1a; transition: transform 0.2s;">' +
                    '    <img class="card__img" src="' + (itemData.poster || '') + '" style="width: 100%; height: 100%; object-fit: cover;" />' +
                    '</div>' +
                    '<div class="card__title" style="margin-top: 8px; font-size: 14px; font-weight: 500; text-align: center; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">' + itemData.title + '</div>' +
                    '</div>');

                card.on('hover:enter', function () {
                    loadAnimeDetails(itemData, movie);
                });
        ...
        this.empty = function (msg) {
            this.reset();
            scroll.append($('<div class="empty" style="text-align: center; padding: 40px; font-size: 1.2em;">' + msg + '</div>'));
    ...
    function initPlugin() {
        ...
        Lampa.Component.add('anitube_view', component);
